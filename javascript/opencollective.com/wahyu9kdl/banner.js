(function () {
  ...
  if (window.OC && window.OC.widgets) {
    window.OC.widgets['banner'] = window.OC.widgets['banner'] || [];
    return;
  }

  window.OC = window.OC || {};
  window.OC.widgets = { 'banner': [] };
  window.addEventListener('message', e => {
    if (e.origin !== 'https://opencollective.com') {
      return;
    }
  ...
  function OpenCollectiveWidget(widget, collectiveSlug, anchor) {
    ...
    this.iframe.src = `https://opencollective.com/${collectiveSlug}/${widget}.html?limit=${limit}&id=${this.id}&style=${style}`;
  ...
  const init = () => {
    ...
    const regex = new RegExp('https://opencollective.com'.replace(/^https?:\/\//, ''), 'i');
